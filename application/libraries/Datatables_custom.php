      ...
      if($sSearch != '')
        for($i = 0; $i < count($mColArray); $i++)
          if ($mColArray[$i]['searchable'] == 'true' && !array_key_exists($mColArray[$i]['data'], $this->add_columns))
            if($this->ci->input->post('search_type') == 'Like' || $this->ci->input->post('search_type') == ''){
              if($this->check_cType())
                $sWhere .= $this->select[$mColArray[$i]['data']] . " LIKE '%" . $sSearch . "%' OR ";
              else
                $sWhere .= $this->select[$this->columns[$i]] . " LIKE '%" . $sSearch . "%' OR ";
            } else if($this->ci->input->post('search_type') == 'Equal'){
              if($this->check_cType()){
                if($i == 1) $sWhere .= $this->select[$mColArray[$i]['data']] . " = '" . $sSearch . "' OR ";
              }
              else {
                if($i == 1) $sWhere .= $this->select[$this->columns[$i]] . " = '" . $sSearch . "' OR ";
              }
            } else {
    ...
    private function produce_output($output, $charset)
    {
      ...
      if($output == 'json')
      {
        ...
        if( isset($_POST['count_id']) ){
          // count normal seperti biasa
          if( $this->ci->input->post('count_id') == NULL ){
            $iFilteredTotal = $this->get_total_results(TRUE);
          } else {
            // count pake summary table count
            $iFilteredTotal = $this->ci->db->select('data_count')->from($this->ci->input->post('table'))
                                            ->where($this->ci->input->post('field'), (int)$this->ci->input->post('count_id'))
                                            ->get()->row()
                                            ->data_count;
          }
        } else {
    ...
    */
    private function get_total_results($filtering = FALSE)
    {
      ...
      if(strlen($this->distinct) > 0)
      {
        ...
        if(isset($this->table_columns[0]))
          $this->ci->db->select($this->table_columns[0]);
      }
